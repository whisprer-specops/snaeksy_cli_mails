...
Write-Host "Debug: Checking path $Path"
Write-Host "Debug: Path exists: $(Test-Path $Path)"

# Function to create directory if it doesn't exist
function Ensure-Directory {
    param ([string]$DirectoryPath)
    
    if (-not (((Test-Path $outputFilePath) -and (Read-Host "File '$outputFilePath' already exists. Overwrite? (Y/N)").ToUpper() -eq 'Y')) {
...
function Secure-Delete {
    ...
    try {
        ...
        # Function to get confirmation with an "All" option
    ...
    foreach ($file in $files) {
        ...
        if ($Operation -eq "Encrypt") {
            if (-not $file.EndsWith(".enc.xml")) {
                ...
                # Process the file
                if (($Force) -or (-not (((Test-Path $outputFilePath) -and (Read-Host "File '$outputFilePath' already exists. Overwrite? (Y/N)").ToUpper() -eq 'Y') -or 
...
try {
    ...
    switch ($Mode) {
        "Encrypt" {
            if (($Force) -or (-not (((Test-Path $outputFilePath) -and (Read-Host "File '$outputFilePath' already exists. Overwrite? (Y/N)").ToUpper() -eq 'Y') -or 
                    ((Test-Path $outputFilePath) -and (Get-OverwriteConfirmation -FilePath $outputFilePath -ConfirmAll ([ref]$confirmAllOverwrite)))) {
